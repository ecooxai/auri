...
test("release metadata stays aligned for Auri v0.84", async () => {
  ...
  assert.equal(packageVersion, "0.84.0");
  assert.equal(tauriVersion, packageVersion);
  assert.equal(cargoVersion, packageVersion);
  assert.equal(lockVersion, packageVersion);
  assert.match(readme, /Current release: \*\*v0\.84\*\* \(package version `0\.84\.0`\)\./);
  assert.match(agentGuide, /Current release: \*\*v0\.84\*\* \(package version `0\.84\.0`\)\./);
});
