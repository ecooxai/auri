...
test("assistant action popup is rendered inside the terminal content pane", async () => {
  ...
  assert.ok(source.includes('<div class="content-pane">${renderActivePanel(state, { native: nativeWebview })}${renderFolderFilePreview(state)}${renderAssistantTranscriptPopup(state)}</div>'));
