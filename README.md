...
Current release: **v0.84** (package version `0.84.0`).
...
- Workspace chips sit horizontally at the top, above the subtab bar, labeled by their number and folder name (`1 projects`, `2 demo` — custom titles appear after the number); click a chip or subtab to focus it. `↑/↓` (or `k/j`) select the workspace, `←/→` or `Tab` select the subtab — the GUI follows when mirrored.
- The terminal panel is a full-feature terminal. Click it (or press Enter, `r`, `i`, or `a`) and every key goes straight to the shared shell — interactive programs like `top`, editors, and colors render live inside the panel through a built-in VT emulator, sized to the panel. `Ctrl+B` returns to normal mode and restores the GUI's terminal size. The wheel scrolls history; the panel seeds with the last 100 lines.
...
`auri browser` asks the running app to serve the full Auri UI at `http://127.0.0.1:8899` and opens it in the default web browser (later visits can simply open that address; the GUI command `auri browser` does the same from the prompt). The page is the same frontend as the desktop window: native calls route over a local HTTP bridge and terminal output streams over server-sent events, so terminals, the folder pane, the system monitor, clipboard history, files, and AI all work.

The browser session is a live mirror of the desktop window: it loads the published app state on start and follows every change over an `app-state` event stream, so the GUI, the TUI, and every open browser tab show the same workspaces, subtabs, and focus. Tab, subtab, terminal, web, settings, and info commands issued in the browser are executed by the desktop window (without stealing focus) and flow back through the mirror; typing in a browser terminal writes into the same shell the GUI shows. Web subtabs cannot embed a WebKit view inside a browser page, so opening one opens a plain new browser tab instead (one named tab per subtab, re-used on navigation). The server only accepts loopback connections and rejects non-local `Host` headers.
