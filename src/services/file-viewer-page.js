...
export function fileViewerPageHtml({ resourceUrl = "", mime = "application/octet-stream", title = "File", path = "", text = null, autoplay = false, compact = false, codemirrorModuleUrl = "", threeModuleUrl = "" }) {
  const kind = viewerKindForFile(path || title, mime);
  const safeTitle = escapeHtml(title);
  const compactMedia = compact && (kind === "audio" || kind === "video");
  const data = safeJson({ resourceUrl, mime, title, path, text, kind, extension: extension(path || title), autoplay, compact: compactMedia, codemirrorModuleUrl, threeModuleUrl });
...
<body class="${compactMedia ? "compact-media" : ""}">
<div class="app">
...
function renderAudio(){
  setStage('<section class="audio-card"><p id="media-error" class="muted" role="status"></p><audio id="audio" preload="metadata" src="' + file.resourceUrl + '"></audio><div class="wave-wrap"><canvas id="waveform" aria-label="Audio waveform"></canvas><span id="loop-pill" class="loop-pill" hidden></span></div><div class="media-controls"><button id="play" class="clean-button" type="button">Play</button><button id="back5" class="clean-button" type="button">−5s</button><button id="forward5" class="clean-button" type="button">+5s</button><input id="volume" type="range" min="0" max="1" step="0.01" value="1" aria-label="Volume"><select id="speed" class="speed-select" aria-label="Speed"><option value="0.5">0.5×</option><option value="0.75">0.75×</option><option value="1" selected>1×</option><option value="1.25">1.25×</option><option value="1.5">1.5×</option><option value="2">2×</option></select><span id="time" class="time-readout">0:00 / 0:00</span></div></section>');
