...
export function renderClipboard(state) {
  return `<section class="clipboard-panel"><header class="panel-title"><div><span>▣</span><div><small>HISTORY</small><h2>Clipboard</h2></div></div>${button("↻", "Refresh clipboard", "clipboard-refresh")}</header>
    <div class="clipboard-grid">${state.clipboard.items.length ? state.clipboard.items.map((item) => `<article class="clipboard-card"><div class="clipboard-card-head"><span>${item.kind === "image" ? "◈" : "≡"}</span><time>${new Date(item.createdAt).toLocaleTimeString()}</time></div><pre>${escapeHtml(item.kind === "text" ? previewClipboardText(item.text) : item.path)}</pre><div>${button("⧉", "Copy", "copy-text", `data-value="${escapeHtml(item.text || item.path || "")}"`)}${button("↙", "Insert into focused input", "clipboard-insert", `data-id="${item.id}"`)}</div></article>`).join("") : `<div class="empty-state"><span>▣</span><h2>No clipboard history</h2><p>Refresh after copying text or an image.</p></div>`}</div>
  </section>`;
}
